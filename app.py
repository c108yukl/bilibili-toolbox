...
from bilibili import extract_bvid, parse_cookie, get_danmaku, get_comments, get_all_comments, get_subtitle
        ...
        with redirect_stdout(cap):
            ...
            if get_sub:
                ...
                await get_subtitle(
                    ...
                    lan_code=lan_code,
                    save_fmt={"srt": "srt", "ass": "ass", "lrc": "lrc", "json": "json", "txt": "srt", "csv": "srt"}.get(
                        save_fmt, "srt"
                    ),
                )
            if get_cm:
    ...
    try:
        ...
        if save_fmt:
            for suffix in [save_fmt, "txt", "json", "csv", "srt", "ass", "lrc"]:
                for pattern in [f"danmaku_{real_bvid}", f"comments_{real_bvid}", f"subtitle_{real_bvid}"]:
