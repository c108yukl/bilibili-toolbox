...
- **🎬 打开即知视频**：弹窗打开自动获取当前视频标题、播放/弹幕/评论数与 UP 主信息卡片（无需跑任务）
- **🔴 直播间支持（部分）**：在 live.bilibili.com 打开弹窗自动显示房间信息（标题/主播/分区/人气/开播状态）；实时弹幕监听因 B站风控升级（-352）**暂停支持**，研究结论与重启方案见 `docs/live-danmu-notes.md`
- **🪟 窗口圆角透明**：弹窗四角透明圆角化（html 透明 + body 圆角），Aurora 版附带毛玻璃通透效果
...
styles/options-theme.css 设置页风格主题层（Editorial/Neumorphism 跟随「界面风格」设置）
icons/         扩展图标
```
...
> 设置页本身也会跟随所选风格切换外观（`options-theme.css` 主题层），点选风格即时预览，
> 「预览弹窗」按钮可在新标签页打开所选风格的弹窗页面。
>
